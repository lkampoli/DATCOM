      SUBROUTINE LINFOR(CNALIN,CYBLIN)
C***********************************************************************
C
C     THIS SUBROUTINE CALCULATES A CONTRIBUTION TO THE INLET LINEAR
C     AERODYNAMICS BASED ON ASPECT RATIO.  THE CN-ALPHA AND CY-BETA
C     IS CALCULATED AS A FUNCTION OF INLET SPAN IN THE APPLICABLE FLIGHT
C     PLANE AND BODY DIAMETER.  THE PROGRAM ALSO CALCULATES THE ASPECT
C     RATIO OF THE INLETS (SPAN**2/S PLAN).
C
C***********************************************************************
C
      COMMON /INLETN/ NIN,INTYPE,XINLT,XDIV,HDIV,LDIV,PHI(20),
     1                XI(5),HI(5),WI(5),ICOVER,RAMANG,IAD,MFR(20)
      COMMON /ILTGEO/ LENGTH(5),LLNGTH(5),ALPEFF(5),SI(5),
     1                CRXLEN(5),DEQI(5),XMIDI(5),XMIDD(3),
     2                XD(3),HD(3),WD(3),DEQD(3),
     3                RARC(5),DLEN(3),KWA(20),KWB(20),
     4                RA2(20),RB2(20),AREAI(5),AREAD(3)
      COMMON /FLC/    NALPHA,ALPHA(20),BETA,PHIROL,NMACH,MACH(20),
     1                ALT(20),REN(20),FLDUM(60)
      COMMON /REFQN/  SREF,LREF,BREF,ROUGH,XCG,ZCG,SCALE,BLAYER,RHR
      COMMON /CONST/  PI,RAD,UNUSED,KAND
      COMMON /POTENT/ CNP(20),CYP(20),CMP(20),CLNP(20)
      COMMON / MOMEN/ CNMOM(20),CYMOM(20)
      COMMON /LINEAR/ ARA,ARB,CNLIN(20),CMLIN(20),
     1                CYLIN(20),CLNLIN(20)
      COMMON /CFFLOW/ CNCF(20),CYCF(20),
     1                CMCF(20),CLNCF(20)
      COMMON /ILTVSC/ THETAI(5,20),PHIR(20),CRXIWD(5,20),
     1                CRXIHT(5,20),CNNIZP(5,20),
     2                CNNIZN(5,20),CNNIYP(5,20),
     3                CNNIYN(5,20),PANILN(5,5,20),
     4                CNCIRN,FRI,RARX(3),
     5                DLCNAP(3,20),DLCNAN(3,20),
     6                DLCYAP(3,20),DLCYAN(3,20)
      COMMON /DIVVSC/ THETAD(5,20),CRXDWD(3,20),
     1                CRXDHT(3,20),CNNDZP(3,20),
     2                CNNDZN(3,20),CNNDYP(3,20),
     3                CNNDYN(3,20),PANDLN(5,20),FRD
      COMMON /BODVSC/ DLCNBP(5,20),DLCNBN(5,20),
     1                DLCYBP(5,20),DLCYBN(5,20),FRB
C
      REAL NIN,INTYPE,LENGTH,LLNGTH,LREF,LDIV,MACH,KWA,KWB
      REAL NALPHA,NMACH
      LOGICAL ICOVER
      COMMON /GEOBOD/ DUM(32),DMAX,SMAX,LBOD,GDUM(12)
      REAL LBOD,LEMAX,LOMAX
      RBOD=DMAX/2.0
C
      RIMAX = RBOD
      LEMAX = RBOD
      UPMAX = RBOD
      LOMAX = RBOD
      DBOD  = 2.*RBOD
C
      NUMI=NIN+0.5
      DO 10 INLET = 1,NUMI
        IF (SIN(PHI(INLET)/RAD).GE.0.0) THEN
          IF (RA2(INLET).GT.RIMAX) RIMAX = RA2(INLET)
        ELSEIF (SIN(PHI(INLET)/RAD).LT.0.0)THEN
          IF (RA2(INLET).GT.LEMAX) LEMAX = RA2(INLET)
        ENDIF
        IF (COS(PHI(INLET)/RAD).GE.0.0) THEN
          IF (RB2(INLET).GT.RIMAX) UPMAX = RB2(INLET)
        ELSEIF (COS(PHI(INLET)/RAD).LT.0.0)THEN
          IF (RB2(INLET).GT.LEMAX) LOMAX = RB2(INLET)
        ENDIF
   10 CONTINUE
      DA2 = RIMAX + LEMAX
      DB2 = UPMAX + LOMAX
C
      CNALIN = PI/(2*SREF)*(DA2-DBOD)**2
      IF (DA2.LT.DBOD) CNALIN = 0.0
      CYBLIN = PI/(2*SREF)*(DB2-DBOD)**2
      IF (DB2.LT.DBOD) CYBLIN = 0.0
C
      ARA = (DA2-DBOD)/XI(5)
      ARB = (DB2-DBOD)/XI(5)
C
C
      RETURN
      END

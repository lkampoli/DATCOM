...
C
      ...
      LOGICAL ICOVER
C
      DIMENSION DCNI(4),DCMI(4)
C
      IF (.NOT.ICOVER) THEN
        WII=2.*LENGTH(1)*TAN(RAMANG/RAD)
        DCNI(1)=(PI*ALP)/(2.*RAD*SREF)*(WII)**2
        DCMI(1)=-((PI*ALP)/(6.*RAD*LREF*SREF))*(2.*WII**2)*LENGTH(1)
      ELSE
        DCNI(1)=((PI*ALP)/(2.*RAD*SREF))*(DEQI(2)**2-DEQI(1)**2)
        DCMI(1)=-((PI*ALP)/(6.*RAD*LREF*SREF))*(2.*DEQI(2)**2
     &          -DEQI(2)*DEQI(1)-DEQI(1)**2)*LENGTH(1)
      ENDIF
      DO 10 J=2,4
        DCNI(J)=((PI*ALP)/(2.*RAD*SREF))*(DEQI(J+1)**2-DEQI(J)**2)
        TCMI=-((PI*ALP)/(6.*RAD*LREF*SREF))*(2.*DEQI(J+1)**2
     &       -DEQI(J+1)*DEQI(J)-DEQI(J)**2)*LENGTH(J)
        IF(DCNI(J).LT.0.00001)THEN
          DCNI(J)=0.0
          DCMI(J)=0.0
        ELSE
          TCNICP=TCMI*LREF/DCNI(J)
          DCNICP=-XI(J)+TCNICP
          DCMI(J)=DCNI(J)*DCNICP/LREF
        ENDIF
   10   CONTINUE
      CNI=0.0
      CMI=0.0

      SUBROUTINE COORD5(X,XU,XL,YUN,YLN,L,I,J,K,II,JJ,KK,III,JJJ,
     1 THN,CAM,RHO,T)
C
C*****
...
C
     ...
     1   -.1015*X(M)**4)
      IF(X(M).LT.ZM)YC=(1./6.)*XK*(X(M)**3-3.*ZM*X(M)**2+ZM**2*(3.-ZM)
     1   *X(M))
      IF(X(M).LT.ZM)ALPHA=ATAN((1./6.)*XK*(3.*X(M)**2-6.*ZM*X(M)+ZM**2
     1   *(3.-ZM)))
 ...
 1020 RK=(3.*((ZM-ZP)**2)-ZM**3)/((1.-ZM)**3)
     ...
     1   -ZM**3*X(M)+ZM**3)
      IF(X(M).LT.ZM)ALPHA=ATAN((1./6.)*XK*(3.*(X(M)-ZM)**2-RK*(1.-ZM)**3
     1   -ZM**3))
      IF(X(M).EQ.ZP)YC=AI*.01
      IF(X(M).EQ.ZP)ALPHA=0.0
      IF(X(M).GT.ZM)YC=(1./6.)*XK*(RK*(X(M)-ZM)**3-RK*X(M)*(1.-ZM)**3
     1   -X(M)*ZM**3+ZM**3)
      IF(X(M).GT.ZM)ALPHA=ATAN((1./6.)*XK*(3.*RK*(X(M)-ZM)**2-RK*(1.-ZM)
     1   **3-ZM**3))

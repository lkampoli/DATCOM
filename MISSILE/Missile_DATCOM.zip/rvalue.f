...
C
C***  SUBROUTINE TO TEST FOR REAL VALUES AND SYNTAX
C
      COMMON /CONST/  PI,RAD,UNUSED,KAND
      DIMENSION KOL(80), NUMBER(15)
      INTEGER FIRST, POWR
      LOGICAL END, SIGN, EXP, STAR, DEC, FLAG
      CHARACTER*4 KOL,BLANK,COMMA,NUMBER,KAND
C
 1000 CONTINUE
      END  = .FALSE.
      SIGN = .FALSE.
 ...
 1050 CONTINUE
     ...
     1         (KOL(L-1) .EQ. NUMBER(14))
        ...
        GO TO 1110
 1060 CONTINUE
      IF(J .GT. 13) GO TO 1070
        IF(DEC) NF = NF+1
        IF(EXP) NF = NF+1
        DEC = .TRUE.
        GO TO 1110
 1070 CONTINUE
      IF(J .GT. 14) GO TO 1080
        IF(STAR) NF = NF+1
        IF(SIGN) NF = NF+1
        IF(EXP)  NF = NF+1
        IF(DEC)  NF = NF+1
        ...
        GO TO 1110
 1080 CONTINUE
      IF(J .GT. 15 .OR. L .EQ. FIRST) GO TO 1090
        IF(.NOT. DEC) NF = NF+1
        IF(EXP)       NF = NF+1
        EXP  = .TRUE.
        SIGN = .FALSE.
        GO TO 1110
 1090 CONTINUE
      IF(KOL(L) .NE. BLANK) GO TO 1100
        NF = NF+1
        GO TO 1110
 1100 CONTINUE
      IF(KOL(L) .EQ. KAND .AND. L .EQ. FIRST) GO TO 1130
      IF(KOL(L) .NE. COMMA .AND. KOL(L) .NE. KAND) GO TO 1130
        END = .TRUE.
C
C***  NUMERIC CHARACTER FOUND
C
 1110 CONTINUE
      IF(L .EQ. FIRST .AND. EXP) GO TO 1130
      IF(END) GO TO 1120
        L = L+1
        GO TO 1020
 1120 CONTINUE
